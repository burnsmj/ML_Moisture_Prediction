### MSI Spectra Machine Learning -- Genotype
### Michael Burns
### 2020-07-08
...
for(n in 1:20) {
  ...
  for(d in c(1,3,5,7,9)) { # Use [[d]] for training and [[d+1]] for validation.
    ...
    spectra_geno_results <- foreach(i = 1:141, .combine = rbind) %dopar% {
      ##########
      # Models #
      ##########
      set.seed(n)
    ...
    spectra_geno_results_tibble <- spectra_geno_results %>%
      as_tibble() %>%
      unnest(cols = everything())
    
    Spectra_Geno_Results <- bind_rows(Spectra_Geno_Results, spectra_geno_results_tibble)
  }
}
...
Spectra_Geno_Results %>% 
  ...
  write_csv("../Results/spectra_genosplit_results_clean.csv")

Spectra_Geno_Results %>%
  write_csv("../Results/spectra_genosplit_results_raw.csv")
