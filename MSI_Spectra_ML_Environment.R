### MSI Spectra Machine Learning -- Environment
### Michael Burns
### 2020-07-08
...
for(n in 1:20) {
  ...
  for(d in c(1,3,5,7,9)) { # Use [[d]] for training and [[d+1]] for validation.
    ...
    train_data <- train_data[,-nearZeroVar(train_data)] # removes environment, and one column when max normalization is used
    valid_data <- valid_data[,-nearZeroVar(valid_data)]
    
    num_cores_limit <- 47
    Spectra_Env_Results <- tibble()
    ...
    spectra_env_results <- foreach(i = 1:141, .combine = rbind) %dopar% {
      iprint <- paste("i = ", i, sep = "")
      print(iprint)
      ##########
      # Models #
      ##########
    ...
    spectra_env_results_tibble <- spectra_env_results %>%
      as_tibble() %>%
      unnest(cols = everything())
    
    Spectra_Env_Results <- bind_rows(Spectra_Env_Results, spectra_env_results_tibble)
  }
}
...
Spectra_Env_Results %>% 
  ...
  write_csv("../Results/spectra_envsplit_results_clean.csv")

Spectra_Env_Results %>%
  write_csv("../Results/spectra_envsplit_results_raw.csv")
