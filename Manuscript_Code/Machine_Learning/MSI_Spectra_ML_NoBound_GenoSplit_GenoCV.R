### MSI Spectra Machine Learning -- Genotype Splitting | Genotype CV | No Prediction Bounds
### Michael Burns
### 2020-07-08
...
##############################
spectra_geno_results <- foreach(n = n_seq, .combine = rbind) %:%
  foreach(d = c(1,3,5,7,9), .combine = rbind) %:% # Use [[d]] for training and [[d+1]] for validation.
  foreach(i = 1:141, .combine = rbind) %dopar% {
    #########################
    ...
    sq_train <- training_set %>%
      ...
      mutate(sum_sq_abs = sum((Absorbance)^2),
             Norm_Abs = Absorbance / sqrt(sum_sq_abs)) %>%
    ...
    spectra_svmr <- train(Moisture_Uptake ~ .,
                          ...
                          method = "svmRadial",
                          metric = "Rsquared",
                          tuneGrid = expand.grid(C = ((i^2) * 0.007), sigma = seq(0.001,0.01,0.1)),
                          trControl = trainControl(method = "cv",
                                                   index = groupKFold(train_data$Genotype, k = 10), 
                                                   savePredictions = T, 
                                                   #predictionBounds = c(T,T),
...
spectra_geno_clean_results <- spectra_geno_results %>%
  as_tibble() %>%
  ...
  group_by(Model, Metric, dataset, Data, Intercept, Mtry, Ncomp, C) %>%
...
###################################
# Writing Multiple Possible Files #
###################################
# The purpose of this section is to add a number to the end of a file name if it 
# already exists. This way I can run the bash script multiple times and get 
# muliple files that I can combine later.
spectra_geno_clean_results %>%
  write_csv(paste("../Results/spectra_genosplit_genocv_nobound_clean_", iter, ".csv", sep = ""))


spectra_geno_results %>%
  as_tibble() %>%
  unnest(cols = everything()) %>%
  write_csv(paste("../Results/spectra_genosplit_genocv_nobound_raw_", iter, ".csv", sep = ""))
