...
n5000_lmer_data_traits <- read_csv("../Data/N5000_Master_With_Predictions.csv") %>%
  select(c(1:10, 152))

registerDoParallel(cores = 4)
trait_var_source_n5000 <- foreach(i = 6:11, .combine = rbind) %dopar% {
  trait_lmer <- lmer(n5000_lmer_data_traits[[i]] ~ (1 | Genotype) + (1 | Env/Rep) + (1 | Rep/Block) + (1 | Genotype:Env), 
                     data = n5000_lmer_data_traits, REML = T)
  ...
  var_exp <- summary(trait_lmer)[[13]] %>%
    ...
    select(Parameter, var, Percent_Var) %>%
    full_join(var_source) %>%
    mutate(Trait = colnames(n5000_lmer_data_traits[i]))
...
```
Ask Jonathan if he only used the subset that had genotypic data.

```{r jonathan's heritability calculations}
registerDoParallel(cores = 4)
trait_var_source_n5000_j <- foreach(i = 6:11, .combine = rbind) %dopar% {
  trait_lmer <- lmer(n5000_lmer_data_traits[[i]] ~ (1 | Genotype) + (1 | Env) + (1 | Env/Rep) + (1 | Rep/Block) + (1 | Genotype:Env), 
                     data = n5000_lmer_data_traits, REML = T)
  
  model_variances <- as.data.frame(VarCorr(trait_lmer))
  h2 <- model_variances$vcov[2]/(model_variances$vcov[2]+(model_variances$vcov[1]/5)+(model_variances$vcov[8]/10))
  
  return(list(
    Trait = colnames(n5000_lmer_data_traits[i]),
    h2 = h2
  ))
}
stopImplicitCluster()

trait_var_source_n5000_j
```
