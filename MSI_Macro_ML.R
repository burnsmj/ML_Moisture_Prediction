...
dataset <- read_csv("../Data/Moisture_Uptake_Master_Dataset_Cook_Macro_Spectra.csv") %>%
  select(Sample_ID, Env, Genotype, Protein_As_is, Fat_As_is, Fiber_As_is, Ash_As_is, Starch_As_is, Moisture_Uptake)
...
Macro_Random_Results %>% 
  ...
  write_csv("../Results/macro_randomsplit_results_clean.csv")
  
Macro_Random_Results %>%
  write_csv("../Results/macro_randomsplit_results_raw.csv")

print('Randomly Split Models Finished')

##############################
# Splitting Data By Genotype #
##############################
...
Macro_Genotype_Results <- tibble()

for(n in 1:100) {
  genotype_macro_results <- foreach(i = 1:iterations, .combine = rbind) %dopar% {
    ##################
    # Splitting Data #
    ##################
  ...
  genotype_macro_results_tibble <- genotype_macro_results %>%
    as_tibble() %>%
    unnest(cols = everything())
  
  Macro_Genotype_Results <- bind_rows(Macro_Genotype_Results, genotype_macro_results_tibble)
...
Macro_Genotype_Results %>% 
  ...
  summarise(Mean_Degree = mean(Degree),
            ...
            Splitting = "Genotype") %>%
  arrange(desc(Mean_Performance)) %>% # arranging data from highest performance to lowest
  write_csv("../Results/macro_genosplit_results_clean.csv")

Macro_Genotype_Results %>%
  write_csv("../Results/macro_genosplit_results_raw.csv")

print('Genotype Split Models Finished')

#################################
# Splitting Data By Environment #
#################################
...
Macro_Env_Results <- tibble()

for(n in 1:100) {
  env_macro_results <- foreach(i = 1:iterations, .combine = rbind) %dopar% {
    ##################
    # Splitting Data #
    ##################
    env <- sample(1:2,1)
  ...
  env_macro_results_tibble <- env_macro_results %>%
    as_tibble() %>%
    unnest(cols = everything())
  
  Macro_Env_Results <- bind_rows(Macro_Env_Results, env_macro_results_tibble)
...
Macro_Env_Results %>% 
  ...
  arrange(desc(Mean_Performance)) %>% # arranging data from highest performance to lowest
  write_csv("../Results/macro_envsplit_results_clean.csv")

Macro_Env_Results %>%
  write_csv("../Results/macro_envsplit_results_raw.csv")

print('Environment Split Models Finished')
